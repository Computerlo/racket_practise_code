...
(define (racketlist->mupllist racket-list)
    (cond [(null? racket-list) (aunit)]
          [(= (length racket-list) 1) (apair (car racket-list) (aunit))]
          [#t (apair (car racket-list) (racketlist->mupllist (cdr racket-list)))]))

(define (mupllist->racketlist mulp-list)
    (cond [(aunit? mulp-list) null]
          [(aunit? (apair-e2 mulp-list)) (cons (apair-e1 mulp-list) null)]
         ...
         (let ([v1 (eval-under-env (call-funexp e) env)]
            ...
            (if (closure? v1)
                (let ([fun-name (fun-nameopt (closure-fun v1))]
                      ...
                      [bodyOfFun (fun-body (closure-fun v1))])
                      (eval-under-env bodyOfFun (append (if fun-name
                                                                  (list (cons fun-name v1))
                                                                   null)
                                                       (cons (cons argument-name v2) (closure-env v1)))))
                (error "It is not a clousure")))]
        [(apair? e)
...
(define (ifeq e1 e2 e3 e4)
    (mlet* (list (cons "value1" e1)(cons "value2" e2))
       (ifgreater (var "value1") (var "value2") e4
                  (ifgreater (var "value2") (var "value1") e4 e3))))
